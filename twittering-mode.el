...
(require 'twittering-proxy)
(require 'twittering-url-shortener)
(require 'twittering-urllib)
(require 'twittering-utilities)
(require 'twittering-wget)
(require 'url)
(require 'xml)
...
(defun twittering-normalize-raw-status (raw-status &optional ignore-retweet)
  (let* ((status-data (cddr raw-status))
	...
	`(,@(mapcar
	  ...
	  ;; Media
	  ,(let ((media-data (assq 'media
				   (cddr (assq 'extended_entities status-data)))))
	     (media (mapcar
		     ;; for now just pull the URL and ignore metadata
		     ;;
		     ;; TODO: support image captions
		     (lambda (x) (assq 'media_url_https x))
		     media-data)))
	  ;; Source.
	  ,@(let ((source (twittering-decode-html-entities
...
to JSON objects from ordinary timeline and search timeline."
  (let* ((encoded-text
	 ...
	 (media (cdr (assq 'media extended-entities)))
	 (func
